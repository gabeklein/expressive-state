---
title: Instructions
description: Field initializers - set, get, ref, def, has, map - with every overload
---

Instructions are special initializers for class fields. They wire up reactive behavior declaratively.

```ts
import { set, get, ref, def, has, map } from '@expressive/react';
```
...
An owned reactive collection - an ordered list of values, or a pool of members the state spawns.

```ts
import { has } from '@expressive/react';
...
`has()` is a proper instruction: the field resolves when the hosting state activates, which adopts the collection in the same step. It is not usable standalone, and the field is read-only - assigning over it throws.

The argument selects one of two modes. Nothing, a falsy value (`null`/`false`, for conditional init), or an iterable makes an ordered list (`has.List<T>`) addressed by index. A `State` class or a factory makes a pool (`has.Pool<T, A>`) whose members exist only by being spawned through `add(...args)` - which forwards its arguments to the constructor exactly as `Type.new()` accepts them (or to the factory's own parameters) and returns the member.

A pool takes no initial argument, so seed it from the `new()` hook, which runs once the field has resolved:
...
Ownership follows freshness: a fresh (never-activated) `State` member - one the pool instantiates, or a `new Item()` the factory constructs - is owned and destroyed when deleted, cleared, or when the owner dies, while an already-activated value (`Item.new()`) stays a guest, held but never destroyed. Fresh members are parented to the hosting state and activate inside its context; a member that dies evicts itself.

- Lists are positional: `get(index)` (negative counts from the end), `get(start, end)`, `get(predicate)`, `set(index, value)`, `put(index, ...values)`, `push`, `pop(index?, count?)`.
- Pools address the member itself: `add`, `has(value)`, `delete(value)` - no index reads, no `push`. Repeat `add` of a present value is a no-op.
- List events are positional - replacing one index notifies that index; insert/remove notifies shifted indices plus length. Pool events are by value, plus shape.
- `size` and iteration track length (list) or shape (pool); `map(fn)`, `filter(fn)`, `any(fn)`, `all(fn)` track what they visit.
- `get()` with no arguments returns a shallow snapshot array.
- Shallow - nested State, `map()`, and `has()` values keep their own reactivity when accessed through the collection.
...
`has.List` and `has.Pool` are the runtime classes - mode is class identity, and adapters extend their prototypes for rendering facades. In `@expressive/react` a collection of Components renders directly: `<ul>{this.todos}</ul>`.

> The deep dive - positional events, ownership and eviction, the seeding seam, and the shared read surface - lives at [Arrays & Collections](/docs/guides/collections).
...
The argument selects one of two modes. With no argument or an iterable of entries, it is a plain keyed map (`map.Insert<K, V>`, extending native `Map`) - you name and insert everything via `set(key, value)`. With a factory function, it is a keyed spawning map (`map.Create<A, V>`, keyed by the factory's first parameter): `set(key, ...rest)` invokes the factory with those arguments and stores the result at `key`, replacing (and destroying, if owned) any previous value.
...
- `set`, `delete`, and `clear` notify affected keys; adding or removing keys also notifies shape.
- `get()` with no key returns a shallow `ReadonlyMap` snapshot.
- Shallow - nested State, `map()`, and `has()` values keep their own reactivity when accessed through the map.
...
// MapLike (internal base): key-addressed reads, removal, entry iteration, transforms - no insertion verb

...
Create a child state owned by the parent.

### `use(StateClass)`

Eager creation, shares parent context:

```ts
child = use(ChildState);
```

### `use(StateClass, false)`

Optional child (may be `undefined`):

```ts
maybe = use(ChildState, false);
```

### `use(StateClass, callback)`

With ready callback:

```ts
auth = use(AuthService, (instance) => {
  instance.initialize();
});
```
...
Custom instruction — receives parent state and property key:
